...
contract AppleVault is Ownable, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    uint256 public seedPoolPid = 0;     // Apple Pool pid in masterchef
    ...
    /**
     * @notice Constructor
     * @param _token: Apple token contract
    ...
    function harvest() public whenNotPaused notContract {
        uint256 PendingApple = MasterChef(masterchef).pendingApple(seedPoolPid, address(this));
        MasterChef(masterchef).withdraw(seedPoolPid, 0);
        
        uint256 harvestedAmount = PendingApple > available() ?
                                available() :
                                PendingApple;
